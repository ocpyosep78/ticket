...
class TicketsScriptHook {
    ...
    <script>
        var STATUS = <?php echo json_encode($this->tickets->status); ?>,
            TYPES = <?php echo json_encode($this->tickets->types); ?>,
            SEVERITY = <?php echo json_encode($this->tickets->severity); ?>;
        ...
        $(function() {
            $("#dataTable").dataTable({
                ...
                fnServerParams: function ( aoData ) {
                    aoData.push({name:"user_id", "value": "<?php echo $this->tickets->orca_auth->user->id; ?>"});
                ...
                aoColumns: [
                    ...
                    {mData: function(src,t,v) {
                       return src.project_title + ' <a href="<?php echo site_url('tickets/edit?project_id='); ?>'+src.project_id+'" class="btn btn-mini btn-primary btn-add">add ticket</a>';
                    }},
                    {mData: function(src,t,v) {
            ...
            $('.dataTables_wrapper .toolbar').append(' <select class="input-medium" id="filterType">'+optType+'</select>' +
...
$ticketsHook = new TicketsScriptHook($this);
add_action('page_foot', array(&$ticketsHook, 'tickets_datatable_script'));

$page_title = $this->orca_auth->user->username . '\'s Tickets';
include 'header.php';
include 'pagetop.php';
?>
